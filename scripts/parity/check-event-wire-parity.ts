...
const casesSchema = z.object({
  ...
  cases: z.array(z.object({
    ...
    source: z.enum(['user', 'agent', 'environment']),
    message: z.unknown(),
  }).strict()),
}).strict();
...
const cases = casesSchema.parse(await readJson(resolve(root, 'transpile/wire/event-cases.json')));
const oracle = oracleSchema.parse(await readJson(resolve(root, 'transpile/wire/python-event-oracle.json')));

if (oracle.source.commit !== manifest.commit) {
  throw new Error(
    `Python event oracle is pinned to ${oracle.source.commit}, but the canonical manifest is ${manifest.commit}`,
  );
}
...
for (const testCase of cases.cases) {
  if (caseIds.has(testCase.id)) throw new Error(`Duplicate event case id: ${testCase.id}`);
  caseIds.add(testCase.id);
  const pythonValue = toJsonValue(oracle.results[testCase.id], `Python event ${testCase.id}`);
  const parsed = eventSchema.parse(pythonValue);
  const typescriptValue = canonicalize(
    toJsonValue(JSON.parse(JSON.stringify(parsed)) as unknown, `TypeScript event ${testCase.id}`),
  );
  const differences: Difference[] = [];
  diffValue(pythonValue, typescriptValue, '', differences);
  if (differences.length > 0) {
    mismatches[testCase.id] = {
      python: pythonValue,
...
for (const oracleCase of Object.keys(oracle.results)) {
  if (!caseIds.has(oracleCase)) throw new Error(`Python event oracle contains stale case: ${oracleCase}`);
}
...
if (output.mismatchCount > 0) {
  console.error(
    `SDK event wire parity failed against OpenHands ${manifest.commit}: `
...
console.log(
  `SDK event wire parity passed against OpenHands ${manifest.commit}: ${output.caseCount} events match exactly.`,
);
...
function diffValue(
  ...
  if (pythonValue === undefined) {
    differences.push({
      ...
      kind: 'missing-python',
      typescript: typescriptValue as JsonValue,
    });
    return;
  }
...
function parseArgs(args: readonly string[]): { readonly report?: string } {
  ...
  throw new Error('Usage: check-event-wire-parity.ts [--report <path>]');
}
